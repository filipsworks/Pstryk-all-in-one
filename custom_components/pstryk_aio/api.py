...
def _pick_metric_container(
    payload: Optional[Dict[str, Any]], keys: tuple[str, ...]
) -> Dict[str, Any]:
...
class PstrykApiClientApiKey:
    ...
    async def _request(
        ...
        _LOGGER.debug(
            f"Wysyłanie żądania (API Key in Authorization): {method} {full_url}, params: {params}, headers: {request_headers}"
        )
        ...
        try:
            async with self._session.request(
                ...
                _LOGGER.debug(
                    f"Odpowiedź z {full_url} (status: {response.status}): {response_text_for_error_log[:500]}..."
                )

                if response.status in [401, 403]:
                    _LOGGER.error(
                        f"Błąd autoryzacji ({response.status}) dla {full_url}. Treść: {response_text_for_error_log[:500]}"
                    )
                    raise PstrykAuthError(
                        f"Błąd autoryzacji ({response.status}). Sprawdź Klucz API. Treść błędu API: {response_text_for_error_log[:100]}"
                    )
                ...
                if response.status == 429:
                    ...
                    if match:
                        try:
                            ...
                            pass
                    self._throttle_until[path] = dt_util.utcnow() + timedelta(
                ...
                if "application/json" in response.headers.get("Content-Type", ""):
                    ...
                    _LOGGER.warning(
                        f"Odpowiedź z {full_url} nie jest typu JSON (Content-Type: {response.headers.get('Content-Type')}). Zwracam tekst: {response_text_for_error_log[:200]}"
                    )
            ...
            if err.status not in [401, 403]:  # PstrykAuthError powinien być już rzucony
                _LOGGER.error(
                    f"Błąd odpowiedzi HTTP ({err.status}) dla {full_url}: {getattr(err, 'message', str(err))}. Treść: {response_text_for_error_log[:500]}"
                )
                raise PstrykApiError(
                    f"Błąd API ({err.status}) dla {full_url}: {getattr(err, 'message', str(err))}"
                ) from err
            ...
            _LOGGER.error(
                f"Błąd sieci (ClientError) podczas żądania do {full_url}: {err}"
            )
            ...
            _LOGGER.error(
                f"Nieoczekiwany błąd podczas żądania do {full_url}: {e}", exc_info=True
            )
    ...
    def _normalize_unified_usage_response(
        ...
        for frame in response_data.get("frames", []):
            ...
            meter_values = _pick_metric_container(
                frame, UNIFIED_METER_VALUES_RESPONSE_KEYS
            )
        ...
        summary = _pick_metric_container(
            response_data.get("summary"), UNIFIED_METER_VALUES_RESPONSE_KEYS
        )
        ...
        if normalized_response["fae_total_usage"] is None:
            normalized_response["fae_total_usage"] = _sum_numeric_frames(
                normalized_frames, "fae_usage"
            )
        if normalized_response["rae_total"] is None:
            normalized_response["rae_total"] = _sum_numeric_frames(
                normalized_frames, "rae"
            )
        if normalized_response["energy_balance"] is None:
            normalized_response["energy_balance"] = _sum_numeric_frames(
                normalized_frames, "energy_balance"
            )

        return normalized_response

    def _normalize_unified_cost_response(
        self, response_data: Optional[Dict[str, Any]]
    ) -> Optional[Dict[str, Any]]:
        ...
        for frame in response_data.get("frames", []):
            ...
            if normalized_frame["energy_balance_value"] is None:
                ...
                if isinstance(fae_cost, (int, float)) and isinstance(
                    sold_value, (int, float)
                ):
                    normalized_frame["energy_balance_value"] = round(
                        float(fae_cost) - float(sold_value), 6
                    )

            normalized_frames.append(normalized_frame)

        summary = _pick_metric_container(
            response_data.get("summary"), UNIFIED_COST_RESPONSE_KEYS
        )
        normalized_response: Dict[str, Any] = {
            "resolution": response_data.get("resolution"),
            "frames": normalized_frames,
        ...
        if normalized_response["fae_total_cost"] is None:
            normalized_response["fae_total_cost"] = _sum_numeric_frames(
                normalized_frames, "fae_cost"
            )
        if normalized_response["total_energy_sold_value"] is None:
            normalized_response["total_energy_sold_value"] = _sum_numeric_frames(
                normalized_frames, "energy_sold_value"
            )
        if normalized_response["total_energy_balance_value"] is None:
            normalized_response["total_energy_balance_value"] = _sum_numeric_frames(
                normalized_frames, "energy_balance_value"
            )
    ...
    def _normalize_unified_pricing_response(
        ...
        if normalized_response["price_net_avg"] is None:
            normalized_response["price_net_avg"] = _sum_numeric_frames(
                normalized_frames, "price_net"
            )
        ...
        if normalized_response["price_gross_avg"] is None:
            normalized_response["price_gross_avg"] = _sum_numeric_frames(
                normalized_frames, "price_gross"
            )
    ...
    async def test_authentication(self) -> bool:
        ...
        try:
            ...
            if meter_data is not None:
                _LOGGER.info(
                    f"Test autoryzacji Kluczem API (w Authorization) zakończony pomyślnie. Pobrane dane (fragment): {str(meter_data)[:200]}"
                )
                return True
            else:
                _LOGGER.warning(
                    f"Test autoryzacji Kluczem API (w Authorization): nie udało się pobrać danych (meter_data is None)."
                )
                return False
        except PstrykAuthError as e:
            _LOGGER.error(
                f"Test autoryzacji Kluczem API (w Authorization) nie powiódł się: {e}"
            )
            return False
        except PstrykApiError as e:
            _LOGGER.error(
                f"Test autoryzacji Kluczem API (w Authorization) nie powiódł się (PstrykApiError): {e}"
            )
            return False
        except Exception as e:
            _LOGGER.error(
                f"Nieoczekiwany błąd podczas testu autoryzacji Kluczem API (w Authorization): {e}",
                exc_info=True,
            )
            return False

    async def get_integrations_meter_data_usage(
    ...
    async def get_integrations_meter_data_cost(
        self, resolution: str, window_start: datetime, window_end: datetime
    ) -> Optional[Dict[str, Any]]:
    ...
    async def get_integrations_pricing_data(
        self, resolution: str, window_start: datetime, window_end: datetime
    ) -> Optional[Dict[str, Any]]:
    ...
    async def get_integrations_prosumer_pricing_data(
        self, resolution: str, window_start: datetime, window_end: datetime
    ) -> Optional[Dict[str, Any]]:
        """Pobiera dane cenowe sprzedaży (prosument) z unified-metrics przy użyciu parametru 'pricing'."""
        try:
            response_data = await self._request_unified_metrics(
                metrics="pricing",
                resolution=resolution,
